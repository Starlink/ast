...
- **testfitschan.c**: FITS card padding ignored during assertions to match Fortran string comparison rules. Fixed a `heap-use-after-free` bug in `astStore_` (memory.c) exposed by AddressSanitizer during `astConvert`.  Also carries C-only write cases with no Fortran counterpart, the last of which pins a segfault in `MakeIntWorld` (fitschan.c): writing `fixtures/wcsconv/inputs/lsst2.ast`, whose pixel-to-sky Mapping contains a SplineMap, applied the SIP CD values to `partmat` rows that the failed linearity test had left NULL, whenever NAXIS1/NAXIS2 gave a box of about 1000 pixels or more and FitsTol was tight enough that the fit over it failed.
