...
\subsection{\xlabel{changes}\xlabel{list_of_most_recent_changes}Changes
Introduced in V8.8.0}
The following describes the most significant changes which have
occurred in the AST library between versions V8.7.2 and V8.8.0 (the
current version):
...
integer. The names of these ``8-byte'' interfaces are the same as the
4-byte interfaces but have the digit ``8'' appended to the function name
(before any trailing data type code). These new interfaces are documented
in an extra paragraph entitled \emph{Handling of Huge Pixel Arrays} attached
to the reference documentation for the main function.

\end{enumerate}

c+
Note, all C or C++ code that uses AST should be recompiled to pick up changes made to the header file ast.h.
c-
