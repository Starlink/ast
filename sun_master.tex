...
%  AST's own additions to the Starlink classes; see
...
c+
\stardocnumber      {211.33}
c-
f+
\stardocnumber      {210.33}
f-
\stardocauthors     {R.F. Warren-Smith \& D.S. Berry}
\stardocdate        {11th September 2026}
...
% \texttt{...} cannot be used here because the synopsis may contain a
% paragraph break, which a short font command such as \texttt rejects.
% The extra braces are needed because \sstdiytopic does not group its
% second argument, so an unbraced font switch would apply to the rest of
% the routine description as well.
\providecommand{\sstsynopsis}[1]{\sstdiytopic{Synopsis}{{\ttfamily #1}}}
...
\subsection{\label{ss:changingmapattrs}Changing Attributes of a Mapping}

Many classes of Mapping have attributes that provide values for parameter
used within the transformation. For instance, the ZoomMap class has an
attribute called ``Zoom'' that gives the scalar value by which each
...
cloned or included in another Object (\emph{i.e.} it has a reference
couint of one) - changing the attributes of such a Mapping is allowed,
and will not generate an error.

Note, the Invert attribute of a Mapping is not subject to this rule and
...
individually using Mappings similar to the PermMap and ZoomMap used
...
using a SpecFrame you must choose whether you want the SpecFrame to
...
DSBSpecFrame, which overcomes this difficulty.
...
AST\_CONVERT
f-
function will return a Mapping that implement equation
...
central spectral position ($f_{central}$), and the IF attribute to
...
Usually, a DSBSpecFrame will be used to describe the WCS for a
1-dimensional array of data values measured using a dual-sideband
instrument. The FrameSet for this sort of situation will typically
contain two Frames: the base Frame will be a simple 1-dimensional Frame
...
the DSBCentre and IF attributes that match the settings of the dual
side-band instrument. The Mapping connecting the base (pixel) Frame to
...
the attributes of the parent SpecFrame class - System, StdOfRest, Units,
Epoch, etc.} within the window corresponding to the value of the SideBand
...
c-
f+
AST\_CONVERT and AST\_FINDFRAME
f-
...
Object             - Base class for all AST Objects
   ...
   Mapping         - Inter-relate two coordinate systems
      ...
      SplineMap    - Map 2-d spherical coordinates using a pair of splines
...
\item A bug has been fixed in the FitsChan class that prevented illegal
...
attribute ``Warnings'' and
c+
function ``astWarnings''.
c-
f+
routine ``AST\_WARNINGS''.
f-
...
\item A new class of Mapping called UnitNormMap has been added that converts
...
c+
\item A new method called astAxNorm
c-
f+
\item A new method called AST\_AXNORM
f-
has been added to the Frame class that normalises an array of axis
...
f+
\item The AST\_TIMEADD
f-
c+
\item The astTimeAdd
c-
method of the TimeMap class now include an extra argument that gives the
number of values supplied in the arguments array. Note, any existing code
that uses this method will need to be changed.

f+
\item The AST\_SLAADD
f-
c+
\item The astSlaAdd
c-
method of the SlaMap class now include an extra argument that gives the
number of values supplied in the arguments array. Note, any existing code
that uses this method will need to be changed.

f+
\item The AST\_SPECADD
f-
c+
\item The astSpecAdd
c-
method of the SpecMap class now include an extra argument that gives the
number of values supplied in the arguments array. Note, any existing code
that uses this method will need to be changed.
...
\item - A new class of Mapping called ChebyMap has been added. This is a
Mapping that implements Chebyshev polynomial transformations.

\item A bug has been fixed in the PolyMap class that caused incorrect values
...
the new
c+
astSetPutErr function.
c-
f+
AST\_SETPUTERR routine.
f-
Previously, the new version needed to be linked into the application at
build time.


\item The Frame class now has a new attribute caled DTAI, which can be used
...
\item The TimeMap class has been changed so that some conversions now require
a ``Dtai'' value (\emph{i.e.} the number of leap seconds) to be supplied by the
...
\item The Region class has a new method called astGetRegionDisc, which
returns the centre and radius of a disc that just encloses a
2-dimensional Region.

\item A new subclass of Region called ``Moc'' has been added. A Moc
describes an arbitrary region of the sky in the form of a set of HEALPix
cells.

\item The Bounded attribute defined by the Region class is now always
...
\item The Moc class now supports version 1.1 of the the MOC recommendation.
...
\item The FitsChan class has a new attribute called ForceTab, which is used
...
\item A bug in the way in which the FitsChan class reads FITS-WCS headers
...
\item The KeyMap class has a new method called astMapCopyEntry that can be used
...
\item - A new subclass of Channel called YamlChan has been added. It allows AST
...
\subsection{Changes Introduced in V9.2.4}
The following describes the most significant changes which
occurred in the AST library between versions V9.2.0 and V9.2.4:

\begin{enumerate}

\item A bug has been fixed that could prevent the astWrite method of the
...
\item - The KeyMap class now supports 64 bit integer
c+
(int64\_t)
c-
f+
...
\item The YamlChan class now supports AST ``native'' encoding.
\end{enumerate}


\subsection{Changes Introduced in V9.2.5}
The following describes the most significant changes which
occurred in the AST library between versions V9.2.4 and V9.2.5:

\begin{enumerate}

\item A new method called astNormPoints has been added to the Frame
...
\item The way in which the astRebinSeq functions use the ``wlim'' argument
...
cases where the AST\_\_GENVAR flag is not set.

\item An option ``--with-external-cminpack'' had been added to the configure
...
\subsection{Changes Introduced in V9.2.6}
The following describes the most significant changes which
occurred in the AST library between versions V9.2.5 and V9.2.6:

\begin{enumerate}

\item A bug has been fixed in the FitsChan class that caused an error to be
...
\subsection{Changes Introduced in V9.2.7}
The following describes the most significant changes which
occurred in the AST library between versions V9.2.6 and V9.2.7:

\begin{enumerate}

\item The FitsChan class has a new attribute called FitsRounding, which
...
\subsection{Changes Introduced in V9.2.8}
The following describes the most significant changes which
occurred in the AST library between versions V9.2.7 and V9.2.8:

\begin{enumerate}

\item  The Fortran interface has been changed to fix a bug caused by a
...
size\_t value rather than an int value. AST now tests the version of
gfortran at configure-time, and uses int or size\_t within the Fortran
...
\item  A bug has been fixed in astRebinSeq that could cause NaN values to
...
\subsection{Changes Introduced in V9.2.9}
The following describes the most significant changes which
occurred in the AST library between versions V9.2.8 and V9.2.9:

\begin{enumerate}

\item A bug has been fixed in fitschan.c that could cause an error when
compiling using some versions of clang.

\end{enumerate}


\subsection{Changes Introduced in V9.2.10}
The following describes the most significant changes which
occurred in the AST library between versions V9.2.9 and V9.2.10:

\begin{enumerate}

\item  A bug has been fixed in the astRebinSeq4 wrapper where lbnd and ubnd
had dimensions ndim\_out rather than ndim\_in. Additionally the dimension
...
\item Some memory allocations in PolyMap were corrected.

\end{enumerate}


\subsection{Changes Introduced in V9.2.11}
The following describes the most significant changes which
occurred in the AST library between versions V9.2.10 and V9.2.11:

\begin{enumerate}

\item  A bug in the flagging of permanent memory blocks when AST is configured
--with-memdebug has been fixed.
\end{enumerate}


\subsection{Changes Introduced in V9.2.12}
The following describes the most significant changes which
occurred in the AST library between versions V9.2.11 and V9.2.12:

\begin{enumerate}

\item  A new attribute called IgnoreBadAlt has been added to the FitsChan
...
be reported.
\item A new warning called BadAlt has been added to the FitsChan class. If
...
FITS-WCS header.
\item The FitsChan class now supports 64 bit integer keyword values.
\item The Table class now supports 64 bit integer columns.
\item The FitsTable class now supports 64 bit integer columns.
\item The MinOrder attribute of the Moc class can be higher than MaxOrder
...
\item When adding FITS data or string representation to a Moc object,
cells of greater order than the MaxOrder attribute will be degraded
to that resolution.
\item Tracing the outline of a Moc region (via the surface mesh) has
hopefully been made more reliable with the addition of backtracking.
\item Added a check for an acos parameter being out of range in internal
line length calculation in SkyFrame.
\item Box simplification to Polygon in a SkyFrame checks handedness
...
\subsection{Changes Introduced in V9.2.13}
The following describes the most significant changes which
occurred in the AST library between versions V9.2.12 and V9.2.13:

\begin{enumerate}

\item   A bug has been fixed in the astRate method of the Mapping class,
which could cause numerical instability in the algorithm and thus return
incorrect values.

\end{enumerate}


\subsection{Changes Introduced in V9.2.14}
The following describes the most significant changes which
occurred in the AST library between versions V9.2.13 and V9.2.14:

\begin{enumerate}

\item A bug that could cause astMapSplit to enter an infinite loop has been
  fixed.
\item A memory leak in the Moc class has been fixed.
\item A PolyMap that is equivalent to a simple shift of origin and/or a linear
  scaling on each axis is now simplified to a combination of equivalent
  simpler classes..
\item If a PolyMap contains multiple coefficients that relate to the same
  input axis powers, they are now combined into a single coefficient.
\item Zero-valued coefficients are now ignored by the PolyMap constructor,
  rather than being stored within the PolyMap.
\item The FitsChan class can now create FITS headers containing SIP
...
\subsection{Changes Introduced in V9.3.0}
The following describes the most significant changes which have
occurred in the AST library between versions V9.2.14 and V9.3.0:

\begin{enumerate}

\item A new subclass of Mapping, a SplineMap, has been added, which
represents a 2-in, 2-out Mapping defined by a pair of two-dimensional
B-spline surfaces.

\end{enumerate}

\subsection{Changes Introduced in V9.4.0}
The following describes the most significant changes which have
occurred in the AST library between versions V9.3.0 and V9.4.0:

\subsubsection{Main Changes}

\textbf{IMPORTANT - a change that may break existing code}

\begin{enumerate}
\item Changing an attribute of a Mapping that has been cloned or included in another Object now reports an error, for every Mapping class.
  The section ``Changing Attributes of a Mapping'' (\secref{ss:changingmapattrs}) has always stated that Mappings are immutable in that situation, and that AST reports an error if the rule is broken, but the rule was only enforced by some classes.
  It is now enforced by all of them, so code that changed one of the attributes below through a pointer to a shared Mapping used to succeed silently and will now fail with the AST\_\_IMMUT error status:

\begin{terminalv}
IntraMap    IntraFlag
MathMap     Seed, SimpFI, SimpIF
PolyMap     IterInverse, NiterInverse, TolInverse
SplineMap   InvNiter, InvTol, OutUnit
\end{terminalv}

  ZoomMap, GrismMap, PcdMap, LutMap and SphMap already enforced it and are unchanged.

  The rule exists to protect against code changing a mapping that has already been stored elsewhere, such as in a CmpMap, and unexpectedly changing how that CmpMap behaves.

  If your code reports AST\_\_IMMUT after this change, set the attribute before the Mapping is shared - that is, before it is passed to a constructor such as astCmpMap or astFrameSet, and before any astClone of it is taken.
  If you need to change an attribute of a Mapping that is already shared, take an astCopy of it and change the copy.

  The Invert attribute is not subject to the rule and can still be changed at any time.
\item The IsSimp item written to a Channel when a Mapping is saved now appears on exactly those Mappings that astSimplify produced, rather than depending on which parts of a Mapping tree happened to have been inspected beforehand.
  Historically the serialization format from this library has not been guaranteed to be byte-for-byte reproducible but we are working towards making the serialized form more reproducible to aid in testing and potential ports to other languages.
\end{enumerate}

\subsubsection{Other Changes in Behaviour}

\begin{enumerate}
\item The KeyMap class now writes the entries of a KeyMap to a Channel in key order.
  The order previously followed the internal hash table, so two KeyMaps holding identical content could be written out differently depending on the order in which their entries had been added, and a KeyMap did not necessarily survive a write/read cycle unchanged.
  Code that compares saved KeyMaps byte for byte will see a different, but now stable, ordering.
  The SortBy attribute is unaffected - it has never influenced the order in which entries are saved.
\item Floating point values are now rounded to integers using round() throughout the library, in place of casts of the form (int)(x+0.5).
  Such a cast truncates towards zero, so negative values were rounded up by one.
  The visible consequences are in the KeyMap class, where an entry holding -2.0 read as an integer used to be returned as -1, and in the FitsChan class, where negative grism interference orders were corrupted.
\item The test used within AST for equality of two floating point values now applies an absolute tolerance of 1.0E-12 alongside the existing relative tolerance, whichever is the larger.
  The absolute floor was previously DBL\_MIN, which is small enough to have no effect, so two values that were themselves of the order of the double precision epsilon were reported as unequal however close together they were.
  Comparisons of values of order unity are unchanged.
  The main effect is that the Equal methods of the Mapping classes no longer report two Mappings as different because of residuals at the level of the last few bits, which can differ between platforms.
\end{enumerate}

\subsubsection{New Features}

\begin{enumerate}
\item The YamlChan class can now read, evaluate and write the ASDF ``divide'' transform, and can read and write the ASDF/GWCS ``spherical\_cartesian'' transform.
\item A new protected method astGetKeyMap has been added to the Object class, which returns a KeyMap that is associated with the Object and created on demand.
  A companion method astHasKeyMap tests whether such a KeyMap already exists, without creating one.
\item Added a new README.md file to provide simple instructions on how to build and test the library.
\item Added an experimental PLplot graphics library plugin, covering both the 2-dimensional grf interface and the 3-dimensional grf3d interface, to allow graphics testing without requiring PGPLOT.
  The text bounding boxes on Linux are not as accurate as those from PGPLOT due to a limitation of the PLplot API.
\item Clarified the license situation by removing historical files.
  This library is licensed with the Gnu Lesser General Public License.
\end{enumerate}

\subsubsection{Bug Fixes}

\begin{enumerate}
\item Many bugs have been fixed in the FitsChan class, most of them found by the greatly expanded test suite.
  Those that are visible to a user of the class are:

  \begin{itemize}
  \item Fractional seconds in a DATE-OBS value were read without regard to the width of the field, so leading zeros were lost and a value such as ``\ldots{}T08:38:39.087'' was read as 0.87 seconds.
    This put an error of up to a second into the Epoch of the FrameSet read from the header.
  \item A negative grism interference order in PVi\_1 was rounded towards zero, so an order of -1 became degenerate and the header was rejected, while an order of -2 silently gave the wrong dispersion.
  \item A WCSAXES value written by a floating point formatter as marginally less than an integer was truncated rather than rounded.
  \item When a WCS was read from a header containing repeated keywords (for instance a header holding the cards of two HDUs), only some of the duplicates were consumed, leaving a partial and self-inconsistent WCS description behind.
    Every copy of a repeated keyword is now consumed.
  \item Legacy six-digit CDjjjiii cards were read and discarded under every encoding, but only translated into PCj\_i under FITS-IRAF.
    They are now left untouched, and so survive in the output, under the other encodings.
  \item Ecliptic axes labelled ELON/ELAT were not recognized.
  \item Reading a FITS-CLASS header failed if alternate axis descriptions were present, and RESTFREQ was mishandled for Galactic coordinates.
  \item Precision was lost when the SpecFrame reference position (RefRA and RefDec) was derived from a header, because the values were formatted using the default number of digits.
  \item 64-bit integer keyword values did not survive being written to and read back from a Channel.
  \item Wide floating point values could be written one column to the right of the intended field, and, in some cases, a number was converted to a string with a leading space.
  \item Writing a FrameSet containing a SIP distortion could produce a header that no FITS-WCS reader can use.
    If the inverse transformation was undefined at the reference point, the SIP coefficients were written without any CTYPE, CRPIX or CD cards, and astWrite reported success.
    In addition, the Mapping following the SIP polynomial was tested for linearity over a box centred on the SIP reference point rather than over the image, with FitsTol applied in the output space of that Mapping rather than in pixels, so a Mapping that was far from linear over the image could be accepted.
    The test is now made over the region the Mapping is actually used for, a SIP description without a usable reference pixel is rejected and the Mapping is instead fitted directly, and astWrite reports failure rather than writing a header holding only alternate axis descriptions when the primary descriptions could not be produced.
  \end{itemize}

  Also fixed are a null pointer dereference and several buffer overruns and memory leaks (in PCFromStore, in WATCoeffs when reading TNX Chebyshev headers, on error paths, and when reading ill-conditioned headers), an out-of-bounds read when scaling the inverse coefficients of a PolyMap on the SIP write path, a null pointer dereference when a non-linear Mapping was written with a FitsTol too tight for the linear fit to succeed, use of the input rather than the output axis count when writing SIP keywords for a non-square Mapping, and double application of the spectral unit factor to CDELT when writing FITS-CLASS.
\item A bug has been fixed in astMask that caused a Polygon defined on the sky and enclosing more than half the sky to be effectively negated.
  astMask and astTranN now agree for such a Polygon.
\item A bug has been fixed in the simplification of two back-to-back SphMaps that cancel: the replacement UnitMap always had two axes, whereas the Cartesian to spherical to Cartesian case requires three.
\item A bug has been fixed in the astRate method of the Mapping class that caused AST\_\_BAD to be returned at x0 = 0 for a Mapping defined only over a narrow domain, such as a ChebyMap covering [-1,1].
  A separate fix corrects an out-of-date array index used when astRate checks whether the gradient over the previous interval was also constant.
\item Several classes did not compare equal to themselves, or compared equal when they should not have done.
  Two identical SpecFluxFrames now compare equal; the UnitNormMap class has gained an Equal method, so two UnitNormMaps with the same centre now compare equal instead of only when they are the same object; and two PermMaps that feed an axis from the same constant slot now compare the constant values, which previously could cause astSimplify to merge two PermMaps that are not equivalent.
\item Comparing two CmpMaps that may not be decomposed, such as CmpMaps that carry an Ident attribute, recursed without limit until the stack was exhausted.
  The comparison now expands the outer level of each CmpMap itself, and compares copies of the components so that the Invert values of Mappings shared with other Objects are left untouched.
\item When astSimplify replaces a WinMap that has unit scale factors by a ShiftMap, a shift term that is zero to within the tolerance used for the scale factors is now treated as zero.
  A shift that had been merged past a MatrixMap could carry a rounding residue of the order of 1E-16, and since a ShiftMap only writes a non-zero shift to a Channel, the same Mapping was saved with a different number of items depending on whether the compiler used a fused multiply-add instruction.
\item A bug has been fixed in the astDat method of the TimeMap class.
  The inverse (TAI to UTC) branch of the pre-1972 ``rubber second'' model used divisors that omitted a factor of the number of seconds in a day, leaking about 164 ms on a UTC to TAI to UTC round trip near MJD 40000.
\item The LSRK, LSRD, Local Group and Galactic velocity calculations in the SpecMap class no longer round the source RA and Dec to single precision before use, recovering about 20 mas of angular precision and about 1 mm/s of velocity.
\item A bug has been fixed in the astAxAngle method of the Frame class that caused AST\_\_BAD to be returned for some degenerate configurations, such as a reference point at the origin with the offset point on the axis being measured.
\item Several bugs have been fixed in the KeyMap class:

  \begin{itemize}
  \item astMapPut1<X> now reports an error if given a zero-length vector.
    Previously this created an entry that claimed to be a scalar but held a null pointer, which could crash astWrite.
  \item astMapGetElem<X> now reports failure for an undefined entry, rather than reporting success without storing anything in the supplied buffer.
    astMapGet0<X> and astMapGet1<X> already did so.
  \item A refused astMapRename no longer discards the entry, leaving the KeyMap holding neither the old key nor the new one.
  \item astMapIterate no longer iterates for ever when the SortBy attribute is set.
  \item Keys copied between two KeyMaps with different KeyCase settings are now looked up and stored using the destination KeyMap's KeyCase.
  \item A locked (MapLocked) KeyMap containing entries can now be written to a Channel and read back.
    Previously the read failed and returned a null Object.
  \item A KeyMap with KeyCase unset no longer acquires it as a set attribute after being written to a Channel and read back.
  \item Converting a stored floating point value to an integer type now saturates at the limits of that type instead of relying on undefined behaviour, a NaN converts to zero, and a numeric string too large for the requested type is treated like any other unparsable string instead of wrapping silently.
  \end{itemize}
\item Several bugs have been fixed in the YamlChan class: the angle units used when writing an AST SphMap to YAML were wrong, reading the null frame in the final step of a GWCS pipeline reported a spurious error, and various other problems in ASDF WCS parsing have been corrected.
\item Some double values in an STC-S description were formatted without using the Channel's own format, so the output could vary between platforms.
\item Three fixes made in later releases of wcslib have been applied to the vendored copy: an uninitialised bound used when checking the validity of the reverse ZPN transformation for polynomials of degree less than two, a boundary condition in the reverse AIT transformation that was too weak and so allowed some invalid pixel coordinates to produce world coordinates, and a more robust method of detecting exact multiples of 90 degrees in the internal sine and cosine functions.
\item Many instances of memory corruption and undefined behaviour have been found and fixed using the new C test suite together with the address, leak and undefined-behaviour sanitizers.
  These include a heap-use-after-free in astStore when the source and destination overlap (which caused astConvert to fail for some Frames), a stack buffer overflow when reading an STC-S AstroCoords description for a multi-axis positional frame, a buffer over-read when the Plot class builds the name of a graphical item, a spurious astFree of a static buffer while reporting an XmlChan error, an uninitialised read in astMapSplit for a MatrixMap containing bad matrix elements, a stack overrun in YamlChan, a read one byte beyond the end of the string passed to AST\_ADDMOCSTRING from Fortran, and assorted buffer overruns and memory leaks in the FitsChan, FitsTable, MocChan and PermMap classes.
\end{enumerate}

\subsubsection{Testing and Build Changes}

\begin{enumerate}
\item Added a new experimental build system based on CMake.
  This build system has not been tested for installations and currently exists primarily to simplify testing without any Starlink software installed.
  The Fortran interface is built only if the AST\_BUILD\_FORTRAN option is enabled.
\item Converted many Fortran test files to C.
  Both the C and the remaining Fortran tests in ast\_tester are now run by ``make check'' in the Autotools build as well as by the CMake build.
  The Fortran tests no longer need the Starlink ERR, CHR, CNF, PSX and PRM libraries, since minimal replacements for the routines they use are provided in ast\_tester/star\_compat.
\item The Autotools build no longer requires a Starlink installation.
  A new bootstrap.local script supplies the few Starlink autoconf macros that configure.ac uses, the Starlink fork of automake is no longer needed, and out-of-source (VPATH) builds and ``make distcheck'' work.
  The Fortran REAL return convention is now measured by configure rather than guessed, and libyaml detection has been improved.
\item The documentation (SUN/210 and SUN/211) can now be built outside a Starlink tree.
  The prologue extraction is done by the starprolog Python package when it is available, falling back to the Starlink getatt and prolat tools otherwise, and the Starlink LaTeX support files are now vendored so that a plain TeX Live installation can typeset the PDFs.
  The hypertext (.htx) documents still need the Starlink star2html tool and are built only when it is present, or can be omitted with the new \texttt{-{}-disable-hyperdocs} configure option.
  In a build outside a Starlink tree the documentation is built only when \texttt{-{}-with-stardocs} is given.
\item The distribution built by ``make dist'' now ships a cut-down set of the test fixtures, sufficient to check that a build works, and the tests that use the missing fixtures are skipped.
  A new ``make fulldist'' target ships the complete fixture corpus for verifying a release.
  A release tarball now includes the PDF documentation.
\item The test suite has been extended with new regression harnesses that compare the results of astTran<X> against stored reference data, check that astSimplify produces the expected Mapping for a large corpus of stored Mappings, and compare FITS round trips and Plot output against reference files.
\item Added a logging graphics plugin that records the drawing calls made by the Plot class and can write them out as SVG, so that graphical output can be compared against reference files without a graphics library being present.
\end{enumerate}

\subsection{Changes Introduced in V9.4.1}

This release only makes changes to the new out-of-Starlink build system to fix some issues with the distribution tar file noticed during the conda build.
In particular the ast\_err.h/AST\_ERR files are now included in the distribution as was previously the case.
A linking fix has also been adopted that was a standalone patch for the
conda feedstock.

\subsection{\xlabel{changes}\xlabel{list_of_most_recent_changes}Changes Introduced in V9.5.0}
The following describes the most significant changes which have
occurred in the AST library between versions V9.4.1 and V9.5.0 (the
current version):

\subsubsection{Main Changes}

\textbf{IMPORTANT - a change in documented behaviour}

\begin{enumerate}
\item The NormUnit attribute of a Frame is now derived from the value of the Frame's Unit attribute, as documented, and equals Unit whenever no simplification is possible.
  Previously NormUnit was derived from the Unit of the underlying Axis, bypassing the Unit values reported by SkyFrame, SpecFrame, TimeFrame and FluxFrame, and reading it raised an error for any Unit that could not be parsed as a units expression (for example a blank Unit, or the sexagesimal descriptions such as ``hh:mm:ss.s'' used by SkyFrame axes).
  For example, a TimeFrame now reports NormUnit ``d'' rather than a blank string, and a SkyFrame axis reports its Unit value unchanged.
  Code that recorded the old NormUnit values will see different results.
\end{enumerate}

\subsubsection{Bug Fixes}

\begin{enumerate}
\item The astEQUAL comparison used internally by AST could overflow for values near the top of the double precision range, causing any value to compare equal to such a value.
  One consequence was that a FrameSet whose spectral axis is described by a LutMap was written to a FitsChan as an empty header, rather than as a -TAB axis, even with TabOK set.
  This was a regression introduced in V9.4.0.
\item Allocation failures while normalising a units string are now reported as errors rather than crashing the library.
\end{enumerate}

\subsubsection{Build System Changes}

\begin{enumerate}
\item On macOS, programs linked with \texttt{-dead\_strip\_dylibs} (as used by conda-forge, among others) no longer lose the graphics and error-reporting satellite libraries, which previously caused a segfault on the first error report.
\item The CMake build now installs the same files as the autotools build: public headers go to include/star (with an AST\_WITH\_TOPINCLUDE option matching \texttt{-{}-with-topinclude}), all libraries are built both shared and static, the ast\_ems, ast\_drama, ast\_pgplot and ast\_pgplot3d libraries are provided, PAL and CMINPACK are built as separate libraries, the macOS library version numbers match those from libtool, and ast\_link and ast\_link\_adam are installed.
\item The CMake build now refuses to configure if stale generated headers from an earlier autotools build are present in the source tree.
\end{enumerate}
